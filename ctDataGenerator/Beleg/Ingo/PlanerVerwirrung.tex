...
Wie können Ausführungspläne noch beschleunigt werden? In PostgreSQL durch Umschreiben nur sehr begrenzt. Es ist zwar möglich, die Queries auf verschiedene Weisen zu schreiben, aber die Schreibweise hat auf den Optimizer nur in seltenen Fällen Einfluss.
Wie sehen diese Fälle aus?
Es gibt Querykonstruktionen\footnote{http://www.postgresql.org/docs/current/interactive/explicit-joins.html}, die den Planner verwirren können und somit zu langsamen Ausführungsplänen führen würden.

Wird der Operator > oder der Operator < verwendet, kann der Planner die Tabellen nicht mehr mit Hilfe eines Hash-JOIN verknüpfen:

\begin{lstlisting}[caption=STRAIGHT JOIN, firstnumber=1, label=STRAIGHTJOIN1]{code:STRAIGHTJOIN1}
...
\begin{lstlisting}[caption=Ausführungsplan nested loop join, firstnumber=1, label=aSTRAIGHTJOIN]{code:aSTRAIGHTJOIN}
...
\begin{lstlisting}[caption=STRAIGHT JOIN, firstnumber=1, label=eq]{code:eq}
...
\begin{lstlisting}[caption=Ausführungsplan hash join, firstnumber=1, label=ahash]{code:ahash}
...
Bei Tabellenspalten, die auf Gleichheit geprüft werden, kann der Planner sich für einen Hash-Join entscheiden. Wenn beide Operanden in dem Vergleich Ergebnis einer Berechnung sind, ist der Planner gezwungen, auf einen Merge-Join zurückzugreifen.

\begin{lstlisting}[caption=STRAIGHT JOIN, firstnumber=1, label=merge]{code:merge}
explain Select u.userId, p.eventid
from public.user u, public.event e, public.participation p
...
\begin{lstlisting}[caption=Ausführungsplan merge join, firstnumber=1, label=amerge]{code:amerge}
...
\begin{lstlisting}[caption=Ausführungsplan nested loop join, firstnumber=1, label=aloop]{code:aloop}
