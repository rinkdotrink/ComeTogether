...
\begin{lstlisting}[caption=kein Hash Join möglich, firstnumber=1, label=keinHashJoin]{code:keinHashJoin}
...
Wird in dem Querie statt mit den Operatoren $>$ und $<$\ mit dem Gleichheitsoperator gearbetet, kann der Planer einen Hash-Join verwenden und das führt in diesem Fall aus zu einem schnelleren Ausführungsplan.

\begin{lstlisting}[caption=Hash Join möglich, firstnumber=1, label=HashJoinMoeglich]{code:HashJoinMoeglich}
